...
consts   <- list(n=n,p=p)
data     <- list(Z=obsModBin,X=XMat,M=M,MQM=MQM,mn=rep(0,p))
inits    <- list(beta1=rnorm(1),beta2=rnorm(1),beta3=rnorm(1),tau=2, 
                 delta=rnorm(p))
################################################################################################
# Nimble Setup
################################################################################################
Rmodel<-nimbleModel(code=binary_PICAR_string, data = data,   # Build Model
...
# Save Data
################################################################################################
save(summaryMat,samples,ptFinal,cvSummary,file="run/nimble/binaryPICAR.RData") # Save Data
