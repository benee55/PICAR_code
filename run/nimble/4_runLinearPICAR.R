################################################################################################
#
# Code to fit linear spatial model using PICAR
#
################################################################################################
################################################################################################
...
consts   <- list(n=n,p=p) # Constants
data     <- list(Z=obsModLinear,X=XMat,M=M,MQM=MQM,mn=rep(0,p)) # Data
inits    <- list(beta1=rnorm(1),beta2=rnorm(1),beta3=rnorm(1),tau=2,  # Initial Conditions
                 delta=rnorm(p),sigma2=1)
################################################################################################
# Nimble Setup
################################################################################################
Rmodel<-nimbleModel(code=linear_PICAR_string, data = data,   # Build Model
...
picarConf$addMonitors(c("beta1", "beta2", "beta3","tau","sigma2","delta")) # Indicate with variables we want to sample
...
# Save Data
################################################################################################
save(summaryMat,samples,ptFinal,cvSummary,file="run/nimble/linearPICAR.RData") # Save Data
