...
iter=10000
m_norm<-stan(file="run/stan/ordinal_PICAR.stan",
             data = list(N=n,
                         K=k,
                         P=p,
                         G=g,
...
thetaMat<-rstan::extract(m_norm, c('theta'))[[1]] # Extract samples for basis coefficients
deltaMat<-rstan::extract(m_norm, c('delta'))[[1]] # Extract samples for cutoffPoints
################################################################################################
# Summarize Samples
################################################################################################
summaryMat<-list()
...
# Save Data
################################################################################################
save(summaryMat,parMat,deltaMat,thetaMat,totTime,cvSummary,
     file="run/stan/ordinalPICARstan.RData") # Save Data
