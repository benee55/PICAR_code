...
load("samples/Binary_Morans.RData") # Mesh File
################################################################################################
...
iter=10000
m_norm<-stan(file="run/stan/binary_PICAR.stan",
             data = list(N=n,
                         K=k,
                         P=p,
                         y=obsModBin,
...
deltaMat<-rstan::extract(m_norm, c('delta'))[[1]] # Extract samples for basis coefficients
################################################################################################
# Summarize Samples
################################################################################################
summaryMat<-list()
...
################################################################################################
mcmcDat<-list()
mcmcDat[[1]]<-parMat; colnames(mcmcDat[[1]])<-c("beta1","beta2","beta3","tau")
mcmcDat[[2]]<-deltaMat
cvSummary<-cvFunction.bin(mcmcDat=mcmcDat,AMatCV=AMatCV,mBase=mBase,
                           XMatCV=XMatCV,obsCV=obsCVBin)
print(cvSummary[[1]]) #MSPE
################################################################################################
# Save Data
################################################################################################
save(summaryMat,parMat,deltaMat,totTime,cvSummary,file="run/stan/binaryPICARstan.RData") # Save Data
